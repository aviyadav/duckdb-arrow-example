...
def main():
    parser = argparse.ArgumentParser(
        description="Generate synthetic dirty event dataset using PyArrow"
    )
    parser.add_argument(
        "--rows",
        type=int,
    ...
    with pv.CSVWriter(out_path, schema) as writer:
        while rows_written < args.rows:
            ...
            print(
                f"  Written {rows_written:,} / {args.rows:,} rows ({(rows_written / args.rows) * 100:.1f}%)"
            )

    elapsed = time.time() - start
    print(
        f"Dataset generation complete! File size: {out_path.stat().st_size / (1024 * 1024):.2f} MB in {elapsed:.2f}s"
    )
